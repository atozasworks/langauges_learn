...
if ($redirectUri === '') {
    $scriptDir = dirname($_SERVER['SCRIPT_NAME'] ?? '/auth-backend/google-callback.php');
...
$redirectWithError = static function (string $errorCode) use ($errorUrl): void {
    header('Location: ' . $errorUrl . '&reason=' . rawurlencode($errorCode));
    exit;
};
...
if (empty($_GET['code']) || empty($_GET['state'])) {
    $redirectWithError('missing_code_or_state');
}
...
if ($tokenResp === false) {
    $redirectWithError('token_request_failed');
}
...
if ($userInfoResp === false) {
    $redirectWithError('userinfo_request_failed');
}

$userInfo = json_decode($userInfoResp, true);
if (empty($userInfo['id']) || empty($userInfo['email'])) {
    $redirectWithError('missing_user_profile');
}
